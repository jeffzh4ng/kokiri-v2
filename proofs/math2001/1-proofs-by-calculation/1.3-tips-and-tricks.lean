...
example {x y : ℚ} (h1 : y + 1 = 3) (h2 : x + 2 * y = 3) : x = -1 :=
  calc
    x = x + 2*y - 2*y := by ring
    _ = 3 - 2*y := by rw [h2]
    _ = 3 - 2*y + 3*y + 1 - 3*y - 1 := by ring
    _ = 3 + (y+1) - 3*((y+1)-1) - 1 := by ring
    _ = 3 + 3 - 3*(3-1) - 1 := by rw [h1]
    _ = 5 - 6 := by ring
    _ = -1 := by ring
...
example {u v : ℚ} (h1 : 4 * u + v = 3) (h2 : v = 2) : u = 1 / 4 :=
  calc
    u = (4*u + v - 3*u -v)/4 + 3*u/4 := by ring
    _ = (3 - 3*u - v)/4 + 3*u/4 := by rw [h1]
    _ = (3 - 3*u -v + 3*u)/4 := by ring
    _ = (3-v)/4 := by ring
    _ = (3-2)/4 := by rw [h2]
    _ = 1/4 := by ring
